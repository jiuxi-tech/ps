	...
	export default {
		...
		methods: {
			...
			handleAdd() {
				...
				this.$refs.TpDialog.show(import('../../../../views/sys/role/ent/add.vue'), param, "新增", options, { action: 'add' });
			},
			// 修改方法
			handleEdit(row) {
				...
				this.$refs.TpDialog.show(import('../../../../views/sys/role/ent/add.vue'), param, "修改", options, { action: 'edit' });
			},
			// 删除方法
			handleDel(row) {
				this.$confirm('确定要删除吗？', () => {
			...
			handleView(row) {
				...
				this.$refs.TpDialog.show(import('../../../../views/sys/role/view.vue'), param, "查看", options, { action: 'view' });
			},
			// 下拉回调
			onSelectChange(e) {
				console.log("下拉选择：" + e);
