...
public class RoleRepositoryImpl extends ServiceImpl<RoleMapper, RolePO> implements RoleRepository {
    ...
    public boolean hasAssociatedUsers(String roleId) {
        // 这里需要查询用户角色关联表
        // 目前简化实现，实际需要根据具体的用户角色关联表来查询
        return roleMapper.checkRoleHasUsers(roleId) > 0;
    }
     ...
     */
    @Override
    public boolean hasChildRoles(String roleId) {
        QueryWrapper<RolePO> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq("parent_role_id", roleId);
        return roleMapper.selectCount(queryWrapper) > 0;
    }
    ...
    private Role toEntity(RolePO rolePO) {
        ...
        role.setRoleType(rolePO.getRoleType() != null ? 
            com.jiuxi.module.auth.domain.model.entity.RoleType.valueOf(rolePO.getRoleType()) : null);
        role.setStatus(rolePO.getStatus() != null ? 
            com.jiuxi.module.auth.domain.model.entity.RoleStatus.fromCode(rolePO.getStatus()) : null);
        role.setBuiltIn(rolePO.getBuiltIn());
        role.setDeptId(rolePO.getDeptId());
        role.setDataScope(rolePO.getDataScope() != null ? 
            com.jiuxi.module.auth.domain.model.entity.DataScope.valueOf(rolePO.getDataScope()) : null);
